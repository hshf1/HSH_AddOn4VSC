...
import { hshRZ, getOS, sethshRZ, setPath, setting_init, getStatusBarItem, initMain } from './init' /** Importiert eine Reihe von Befehlen aus der init.ts */
...
async function initialize() {								/** Der Zweck dieser Funktion ist es, die Module init.ts und github.ts zu initialisieren */
	...
	while (init_status === undefined) {											/** Schleife wird solange ausgeführt bis init_status nicht mehr undefiniert ist */
		if (setting_init !== undefined && getGithubStatus() !== undefined) {		/** überprüft ob init.ts aufgerufen wurde und github.ts aufgerufen wurde*/
